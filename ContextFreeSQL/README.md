...
| Variable | Use |
|---|---|
| `CFS_VERSION` | Install a specific release, e.g. `CFS_VERSION=v0.6.0` |
...
| `--help` | `-h` | Show help |
| `--version` | `-v` | Show the version, e.g. `contextfreesql 0.6.0` |
...
```sql
--Generated by ContextFreeSQL 0.6.0 on 2026-09-17 13:12:34 from localhost.const1
```
...
- **Extensions are created, not upgraded or dropped.** An extension the source has is created on the target
  where it is missing (`CREATE EXTENSION IF NOT EXISTS`, pinned to the schema the source keeps it in), which is
  what makes a `vector` or `citext` column work there. A version difference is left alone, because upgrading is
