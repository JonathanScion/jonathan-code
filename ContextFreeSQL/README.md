...
| Variable | Use |
|---|---|
| `CFS_VERSION` | Install a specific release, e.g. `CFS_VERSION=v0.6.2` |
...
| `--help` | `-h` | Show help |
| `--version` | `-v` | Show the version, e.g. `contextfreesql 0.6.2` |
...
```sql
--Generated by ContextFreeSQL 0.6.2 on 2026-09-17 13:12:34 from localhost.const1
```
...
- **The HTML report and CSV export need a database server that shares your filesystem.** They are written by the
  *server*, with `pg_read_file` and `COPY TO`, not by this tool — so they work against a local PostgreSQL and not
  against a managed one. Azure and RDS grant neither `pg_read_server_files` nor `pg_write_server_files`, and their
