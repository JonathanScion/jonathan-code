...
def create_db_state_schemas(dbtype: DBType, tbl_ents: pd.DataFrame, tbl_schemas: pd.DataFrame, ops_all_schemas: bool, ops_remove_all_extra_ents: bool) ->  tuple[StringIO, StringIO]:
    ...
    if ops_remove_all_extra_ents:
        create_schemas.write(f"{'\t' * ident_level}--Dropping Schemas that need to be dropped:\n")
        ...
        if dbtype == DBType.MSSQL:
            ...
            utils.add_print(dbtype, 4, drop_schemas, "'Dropping Schema ' || temprow.schema_name || ':'")
            utils.add_exec_sql(dbtype, 4, drop_schemas, "'DROP SCHEMA ' || temprow.schema_name || ';'")
            drop_schemas.write(f"{'\t' * ident_level}\t\tEND LOOP;\n")
            drop_schemas.write(f"{'\t' * ident_level}\tEND; --FOR\n")
    ...
    if dbtype == DBType.MSSQL:
        ...
        create_schemas.write(f"{'\t' * ident_level}END; --of adding new entities and ones that were modified\n")

    # PostgreSQL specific BEGIN block
    ident_level -= 1
    if dbtype == DBType.PostgreSQL:
        create_schemas.write(f"{'\t' * ident_level}END; --schema code\n")
