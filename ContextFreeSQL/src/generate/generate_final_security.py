...
def generate_create_roles(db_type: DBType, sql_buffer: StringIO):
    ...
    utils.add_print(db_type, 3, sql_buffer, "'' || CASE WHEN temprow.roleStat = 1 THEN 'Creating role: ' ELSE 'Altering role: ' END || temprow.rolname")

    # Build CREATE/ALTER ROLE statement into v_sql variable
    sql_buffer.write("\t\t\tIF temprow.roleStat = 1 THEN\n")
    sql_buffer.write("\t\t\t\t-- Create new role\n")
    sql_buffer.write("\t\t\t\tv_sql := 'CREATE ROLE ' || quote_ident(temprow.rolname) ||\n")
    ...
    sql_buffer.write("\t\t\t\tv_sql := 'ALTER ROLE ' || quote_ident(temprow.rolname) ||\n")
    sql_buffer.write("\t\t\t\t\tCASE WHEN temprow.rolcanlogin THEN ' LOGIN' ELSE ' NOLOGIN' END ||\n")
    sql_buffer.write("\t\t\t\t\tCASE WHEN temprow.rolsuper THEN ' SUPERUSER' ELSE ' NOSUPERUSER' END ||\n")
    sql_buffer.write("\t\t\t\t\tCASE WHEN temprow.rolcreatedb THEN ' CREATEDB' ELSE ' NOCREATEDB' END ||\n")
    sql_buffer.write("\t\t\t\t\tCASE WHEN temprow.rolcreaterole THEN ' CREATEROLE' ELSE ' NOCREATEROLE' END ||\n")
    ...
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Granting ' || temprow.role_name || ' TO ' || temprow.member_name")
    sql_buffer.write("\t\t\tv_sql := 'GRANT ' || quote_ident(temprow.role_name) || ' TO ' || quote_ident(temprow.member_name) ||\n")
...
def generate_grant_table_permissions(db_type: DBType, sql_buffer: StringIO):
    ...
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Granting ' || temprow.privilege_type || ' ON ' || temprow.table_schema || '.' || temprow.table_name || ' TO ' || temprow.grantee")
    sql_buffer.write("\t\t\tv_sql := 'GRANT ' || temprow.privilege_type || ' ON ' || quote_ident(temprow.table_schema) || '.' || quote_ident(temprow.table_name) ||\n")
    ...
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Granting ' || temprow.privilege_type || ' (' || temprow.column_name || ') ON ' || temprow.table_schema || '.' || temprow.table_name || ' TO ' || temprow.grantee")
    sql_buffer.write("\t\t\tv_sql := 'GRANT ' || temprow.privilege_type || ' (' || quote_ident(temprow.column_name) || ') ON ' ||\n")
    ...
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'' || CASE WHEN temprow.policyStat = 1 THEN 'Creating' ELSE 'Replacing' END || ' RLS policy: ' || temprow.policy_name || ' ON ' || temprow.table_schema || '.' || temprow.table_name")

    sql_buffer.write("\t\t\t-- Drop existing policy if altering\n")
    sql_buffer.write("\t\t\tIF temprow.policyStat = 3 THEN\n")
    sql_buffer.write("\t\t\t\tv_sql := 'DROP POLICY IF EXISTS ' || quote_ident(temprow.policy_name) || ' ON ' ||\n")
    sql_buffer.write("\t\t\t\t\tquote_ident(temprow.table_schema) || '.' || quote_ident(temprow.table_name);\n")
    utils.add_exec_sql(db_type, 4, sql_buffer, "v_sql")
    sql_buffer.write("\t\t\tEND IF;\n")

    sql_buffer.write("\t\t\t-- Create policy\n")
    sql_buffer.write("\t\t\tv_sql := 'CREATE POLICY ' || quote_ident(temprow.policy_name) || ' ON ' ||\n")
    sql_buffer.write("\t\t\t\tquote_ident(temprow.table_schema) || '.' || quote_ident(temprow.table_name) ||\n")
...
def generate_revoke_and_drop_extra_security(db_type: DBType, sql_buffer: StringIO, remove_extra_ents: bool):
    ...
    # Revoke extra TABLE permissions (in DB but not in script)
    sql_buffer.write("\n\t--Revoking Extra Table Permissions (not in script)-----------------------\n")
    sql_buffer.write("\tdeclare temprow record; v_sql text;\n")
    sql_buffer.write("\tBEGIN\n")
    sql_buffer.write("\t\tFOR temprow IN\n")
    sql_buffer.write("\t\t\tSELECT tp.grantee, tp.table_schema, tp.table_name, tp.privilege_type\n")
    sql_buffer.write("\t\t\tFROM information_schema.table_privileges tp\n")
    sql_buffer.write("\t\t\tWHERE tp.grantee NOT LIKE 'pg_%'\n")
    sql_buffer.write("\t\t\t\tAND tp.grantee NOT IN ('postgres', 'PUBLIC')\n")
    sql_buffer.write("\t\t\t\tAND tp.table_schema NOT IN ('pg_catalog', 'information_schema')\n")
    sql_buffer.write("\t\t\t\tAND NOT EXISTS (\n")
    sql_buffer.write("\t\t\t\t\tSELECT 1 FROM ScriptTablePermissions stp\n")
    sql_buffer.write("\t\t\t\t\tWHERE stp.grantee = tp.grantee\n")
    sql_buffer.write("\t\t\t\t\t\tAND stp.table_schema = tp.table_schema\n")
    sql_buffer.write("\t\t\t\t\t\tAND stp.table_name = tp.table_name\n")
    sql_buffer.write("\t\t\t\t\t\tAND stp.privilege_type = tp.privilege_type\n")
    sql_buffer.write("\t\t\t\t)\n")
    sql_buffer.write("\t\t\t\t-- Only revoke for tables we're scripting\n")
    sql_buffer.write("\t\t\t\tAND EXISTS (\n")
    sql_buffer.write("\t\t\t\t\tSELECT 1 FROM ScriptTables st\n")
    sql_buffer.write("\t\t\t\t\tWHERE st.table_schema = tp.table_schema\n")
    sql_buffer.write("\t\t\t\t\t\tAND st.table_name = tp.table_name\n")
    sql_buffer.write("\t\t\t\t)\n")
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Revoking extra permission: ' || temprow.privilege_type || ' ON ' || temprow.table_schema || '.' || temprow.table_name || ' FROM ' || temprow.grantee")
    sql_buffer.write("\t\t\tv_sql := 'REVOKE ' || temprow.privilege_type || ' ON ' || quote_ident(temprow.table_schema) || '.' || quote_ident(temprow.table_name) || ' FROM ' || quote_ident(temprow.grantee);\n")
    utils.add_exec_sql(db_type, 3, sql_buffer, "v_sql")
    sql_buffer.write("\t\tEND LOOP;\n")
    sql_buffer.write("\tEND;\n")
    ...
    sql_buffer.write("\t\t\t\t)\n")
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Revoking extra membership: ' || temprow.role_name || ' FROM ' || temprow.member_name")
    sql_buffer.write("\t\t\tv_sql := 'REVOKE ' || quote_ident(temprow.role_name) || ' FROM ' || quote_ident(temprow.member_name);\n")
    utils.add_exec_sql(db_type, 3, sql_buffer, "v_sql")
    sql_buffer.write("\t\tEND LOOP;\n")
    sql_buffer.write("\tEND;\n")
    ...
    sql_buffer.write("\t\t\t\t)\n")
    sql_buffer.write("\t\tLOOP\n")
    utils.add_print(db_type, 3, sql_buffer, "'Dropping extra role: ' || temprow.rolname")
    sql_buffer.write("\t\t\t-- Note: This will fail if role owns objects. Consider REASSIGN OWNED first.\n")
    sql_buffer.write("\t\t\tv_sql := 'DROP ROLE ' || quote_ident(temprow.rolname);\n")
    sql_buffer.write("\t\t\tBEGIN\n")
    utils.add_exec_sql(db_type, 4, sql_buffer, "v_sql")
    sql_buffer.write("\t\t\tEXCEPTION WHEN dependent_objects_still_exist THEN\n")
    utils.add_print(db_type, 4, sql_buffer, "'  Cannot drop role ' || temprow.rolname || ' - owns objects. Use REASSIGN OWNED BY ' || temprow.rolname || ' TO postgres; first.'")
    sql_buffer.write("\t\t\tEND;\n")
    sql_buffer.write("\t\tEND LOOP;\n")
    sql_buffer.write("\tEND;\n")
