...
def generate_all_script(schema_tables: DBSchema, db_type: DBType, tbl_ents: pd.DataFrame, scrpt_ops: ScriptingOptions, input_output: InputOutput, got_specific_tables: bool, tables_data: ListTables | None = None, sql_script_params: SQLScriptParams | None = None, source_db_label: str = "") -> str:
    ...
    header = build_script_header(db_syntax=db_syntax, scrpt_ops=scrpt_ops, sql_script_params=sql_script_params, filename=script_filename, base_path=base_path, include_base_path=needs_base_path, source_db_label=source_db_label)
    buffer.write(header)
...
def build_script_header(db_syntax: DBSyntax, scrpt_ops: ScriptingOptions, sql_script_params: SQLScriptParams, filename: str, base_path: str = "", include_base_path: bool = False, source_db_label: str = "") -> str:
    header = StringIO()
    ...
    # Which build produced this script, and from where
    header.write(f"--Generated by ContextFreeSQL {__version__} on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    header.write(f"{' from ' + source_db_label if source_db_label else ''}\n")
