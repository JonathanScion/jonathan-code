...
def build_script_header(db_syntax: DBSyntax, scrpt_ops: ScriptingOptions, sql_script_params: SQLScriptParams, filename: str, base_path: str = "", include_base_path: bool = False, source_db_label: str = "", max_rows_per_table: int = 0, db_type: DBType = DBType.PostgreSQL) -> str:
    ...
    if max_rows_per_table > 0:
        # The data here is a sample, so it does not describe the source's full contents
        header.write(f"--WARNING: data is limited to {max_rows_per_table} rows per table (tables_data.max_rows_per_table).\n")
        header.write("--         Run against a database that holds data, this DELETES every row the script does not carry.\n")
