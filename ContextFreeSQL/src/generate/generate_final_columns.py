...
def generate_add_alter_drop_cols(db_type: DBType, sql_buffer, j2_alter_cols_not_null, got_data: bool = False):
    ...
    if db_type == DBType.MSSQL:
        ...
        utils.add_print(db_type, 3, sql_buffer, "'Table ' || temprow.table_schema ||  '.' || temprow.table_name ||': adding column ' || temprow.col_name")
    ...
    if db_type == DBType.MSSQL:
        ...
        sql_buffer.write("\t\t\t\tC.is_identity,\n")
        sql_buffer.write("\t\t\t\tC.is_identity_diff,\n")
        sql_buffer.write("\t\t\t\tC.is_identity_db\n")
        sql_buffer.write("\t\t\tFROM  ScriptCols C\n")
        sql_buffer.write("\t\t\tWHERE colStat = 3\n")
        sql_buffer.write("\t\tLOOP\n")
        utils.add_print(db_type, 3, sql_buffer, "'Table ' || temprow.table_schema ||  '.' || temprow.table_name ||': column ' || temprow.col_name || ' needs to be changed: ' || temprow.diff_descr")
        utils.add_exec_sql(db_type, 3, sql_buffer, "temprow.SQL_ALTER", ent_schema="temprow.table_schema", ent_name="temprow.table_name")
        # Handle identity changes - requires separate ALTER statement in PostgreSQL
        sql_buffer.write("\t\t\t-- Handle identity changes\n")
        sql_buffer.write("\t\t\tIF temprow.is_identity_diff = B'1' THEN\n")
        sql_buffer.write("\t\t\t\tIF temprow.is_identity = true AND (temprow.is_identity_db = false OR temprow.is_identity_db IS NULL) THEN\n")
        sql_buffer.write("\t\t\t\t\t-- Need to add identity\n")
        utils.add_print(db_type, 5, sql_buffer, "'Adding IDENTITY to column ' || temprow.col_name")
        utils.add_exec_sql(db_type, 5, sql_buffer, "'ALTER TABLE ' || temprow.table_schema || '.' || temprow.table_name || ' ALTER COLUMN ' || temprow.col_name || ' ADD GENERATED ALWAYS AS IDENTITY'", ent_schema="temprow.table_schema", ent_name="temprow.table_name")
        sql_buffer.write("\t\t\t\tELSIF temprow.is_identity = false AND temprow.is_identity_db = true THEN\n")
        sql_buffer.write("\t\t\t\t\t-- Need to drop identity\n")
        utils.add_print(db_type, 5, sql_buffer, "'Dropping IDENTITY from column ' || temprow.col_name")
        utils.add_exec_sql(db_type, 5, sql_buffer, "'ALTER TABLE ' || temprow.table_schema || '.' || temprow.table_name || ' ALTER COLUMN ' || temprow.col_name || ' DROP IDENTITY IF EXISTS'", ent_schema="temprow.table_schema", ent_name="temprow.table_name")
    ...
    if db_type == DBType.MSSQL:
        ...
        sql_buffer.write("\t\t\tWHERE colStat = 2\n")
        sql_buffer.write("\t\tLOOP\n")
        utils.add_print(db_type, 3, sql_buffer, "'Table ' || temprow.table_schema ||  '.' || temprow.table_name ||': dropping column ' || temprow.col_name")
        utils.add_exec_sql(db_type, 3, sql_buffer, "temprow.SQL_DROP", ent_schema="temprow.table_schema", ent_name="temprow.table_name")
        sql_buffer.write("\t\tEND LOOP;\n")
        sql_buffer.write("\tEND; --of cursor\n")
    ...
    if got_data:  
        ...
        if db_type == DBType.MSSQL:
            ...
            utils.add_print(db_type, 3, j2_alter_cols_not_null, "'Table ' || temprow.table_schema ||  '.' || temprow.table_name ||': column ' || temprow.col_name || ' needs to be changed to NOT NULL, now that I got the data:'")
            utils.add_exec_sql(db_type, 3, j2_alter_cols_not_null, "temprow.SQL_ALTER_PostData_NotNULL", ent_schema="temprow.table_schema", ent_name="temprow.table_name")
...
def generate_drop_add_defaults(db_type: DBType, j2_defaults_drop, j2_defaults_add):
    ...
    if db_type == DBType.MSSQL:
        ...
        utils.add_exec_sql(db_type, 3, j2_defaults_drop, ent_schema="temprow.table_schema", ent_name="temprow.table_name")
        j2_defaults_drop.write("\n")
        ...
        utils.add_print(db_type, 3, j2_defaults_add, "'Table ' || temprow.table_schema || '.' || temprow.table_name || ': adding default ' || temprow.default_name")
        j2_defaults_add.write("\t\t\tsqlCode := temprow.SQL_CREATE;\n")
        utils.add_exec_sql(db_type, 3, j2_defaults_add, ent_schema="temprow.table_schema", ent_name="temprow.table_name")
